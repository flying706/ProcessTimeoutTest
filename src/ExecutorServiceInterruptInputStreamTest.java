...
/*
    测试超时能否中断
 */
public class ExecutorServiceInterruptInputStreamTest {
    public static void main(String[] args) throws SocketException {
        ...
        FutureTask<Integer> task = new FutureTask<Integer>(new Callable<Integer>(){
            public Integer call() throws Exception {
                ...
                BlockThreadSystemin t = new BlockThreadSystemin(ds);
        ...
        for (Thread thread : threads) {
            ...
            if(thread.getName().equals("Thread-0")){
                thread.interrupt();
                try {
                    System.out.println("start close ds");
                    //使用socket时阻塞能被关闭 ，使用输入流关闭也会被阻塞，这个怎么解决？
                    ds.close();
                    System.out.println("end close ds");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    static class BlockThreadSystemin extends Thread{
        private BufferedReader sin;
        public BlockThreadSystemin(BufferedReader sin){
            this.sin = sin;
        }
        public void run(){
            try {
                String sreadline = sin.readLine();
            } catch (IOException e) {
                e.printStackTrace();
            }finally {
                try {
